...
t01.4 <- round(lm.subset1.estimate$coefficients[2], 3)
...
Based on an examination of our `lm()` results above, we guess that the Output data was simulated with a $\beta_{1}$ or slope of 0.8 for both, and a $\beta_{0}$ or intercept of $\pm$ 0.7. We'll also guess a standard deviation for our residuals of 0.4. So we ascribe to our two models the following equations:

$Output\ 1 = 0.8 \cdot Input + 0.7 + \varepsilon$

$Output\ 2 = 0.8 \cdot Input - 0.7 + \varepsilon$

$\varepsilon \sim Norm(0, 0.4)$
...
subset2.b <- -0.7
subset.eps <- 0.4
subset1.y.fit <- subset.a * sample.data$Input + subset1.b
subset2.y.fit <- subset.a * sample.data$Input + subset2.b
setset1.y <- subset1.y.fit + rnorm(1000, mean = 0, sd = subset.eps)
setset2.y <- subset2.y.fit + rnorm(1000, mean = 0, sd = subset.eps)

plot(sample.data$Input, subset1, ylim = c(-5, 5))
```
